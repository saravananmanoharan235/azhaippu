...
function step3_btn(){ 
	...
	if(jQuery('#form1').find('#vehicle_sno').length >0 && jQuery('#form1').find('#vehicle_sno').val() == ''){
		...
		//var regex = /^([a-zA-Z0-9\u0600-\u06FF\u0660-\u0669]{1,100})$/;
...
function formsubmitfunc(){
	...
	if(jQuery('#form1').find('#photograph').val() == ''){
		...
		if(jQuery('#form1').find('#date_photographinput').val() == ''){
			jQuery('#form1').find('#date_photographinput').addClass('error');
			jQuery('#form1').find('#date_photographinput').next().show();
			error++;
		}else{
			jQuery('#form1').find('#date_photographinput').removeClass('error');
			jQuery('#form1').find('#date_photographinput').next().hide();
		}
	...
	if(jQuery('#form1').find('#driver_license').val() == ''){
		...
		if(jQuery('#form1').find('#driverlic_dd').length > 0){
			//alert();
			if(jQuery('#form1').find('#driverlic_dd').val() == ''){
				jQuery('#form1').find('#driverlic_dd').addClass('error');
				jQuery('#form1').find('#driverlic_mm').addClass('error');
				jQuery('#form1').find('#driverlic_yy').addClass('error');
				jQuery('#form1').find('#driverlic_dd').next().show();
				error++;
			}else{
				jQuery('#form1').find('#driverlic_dd').removeClass('error');
				jQuery('#form1').find('#driverlic_dd').next().hide();
				if(jQuery('#form1').find('#driverlic_mm').length > 0){
					//alert();
					if(jQuery('#form1').find('#driverlic_mm').val() == ''){
						jQuery('#form1').find('#driverlic_mm').addClass('error');
						jQuery('#form1').find('#driverlic_mm').next().show();
						error++;
					}else{
						jQuery('#form1').find('#driverlic_mm').removeClass('error');
						jQuery('#form1').find('#driverlic_mm').next().hide();
						if(jQuery('#form1').find('#driverlic_yy').length > 0){
							//alert();
							if(jQuery('#form1').find('#driverlic_yy').val() == ''){
								jQuery('#form1').find('#driverlic_yy').addClass('error');
								jQuery('#form1').find('#driverlic_yy').next().show();
								error++;
							}else{
								jQuery('#form1').find('#driverlic_yy').removeClass('error');
								jQuery('#form1').find('#driverlic_yy').next().hide();
								var driverlicdate = jQuery('#form1').find('#driverlic_dd').val()+'/'+jQuery('#form1').find('#driverlic_mm').val()+'/'+jQuery('#form1').find('#driverlic_yy').val();
								if(checkdateno(driverlicdate) == 1 && driverlicdate!=''){
									jQuery('#form1').find('#driverlic_dd,#driverlic_mm,#driverlic_yy').addClass('error');
									jQuery('#form1').find('#driverlic_dd').next().next().show();
									error++;
								}else{
									jQuery('#form1').find('#driverlic_dd,#driverlic_mm,#driverlic_yy').removeClass('error');
									jQuery('#form1').find('#driverlic_dd').next().next().hide();
								}
							}
						}
					}
	...
	if(jQuery('#form1').find('#vehicle_reg').val() == ''){
		...
		if(jQuery('#form1').find('#vehiclereg_dd').length > 0){
			//alert();
			if(jQuery('#form1').find('#vehiclereg_dd').val() == ''){
				jQuery('#form1').find('#vehiclereg_dd').addClass('error');
				jQuery('#form1').find('#vehiclereg_mm').addClass('error');
				jQuery('#form1').find('#vehiclereg_yy').addClass('error');
				jQuery('#form1').find('#vehiclereg_dd').next().show();
				error++;
			}else{
				jQuery('#form1').find('#vehiclereg_dd').removeClass('error');
				jQuery('#form1').find('#vehiclereg_dd').next().hide();
				if(jQuery('#form1').find('#vehiclereg_mm').length > 0){
					//alert();
					if(jQuery('#form1').find('#vehiclereg_mm').val() == ''){
						jQuery('#form1').find('#vehiclereg_mm').addClass('error');
						jQuery('#form1').find('#vehiclereg_mm').next().show();
						error++;
					}else{
						jQuery('#form1').find('#vehiclereg_mm').removeClass('error');
						jQuery('#form1').find('#vehiclereg_mm').next().hide();
						if(jQuery('#form1').find('#vehiclereg_yy').length > 0){
							//alert();
							if(jQuery('#form1').find('#vehiclereg_yy').val() == ''){
								jQuery('#form1').find('#vehiclereg_yy').addClass('error');
								jQuery('#form1').find('#vehiclereg_yy').next().show();
								error++;
							}else{
								jQuery('#form1').find('#vehiclereg_yy').removeClass('error');
								jQuery('#form1').find('#vehiclereg_yy').next().hide();
								var driverlicdate = jQuery('#form1').find('#vehiclereg_dd').val()+'/'+jQuery('#form1').find('#vehiclereg_mm').val()+'/'+jQuery('#form1').find('#vehiclereg_yy').val();
								if(checkdateno(driverlicdate) == 1 && driverlicdate!=''){
									jQuery('#form1').find('#vehiclereg_dd,#vehiclereg_mm,#vehiclereg_yy').addClass('error');
									jQuery('#form1').find('#vehiclereg_dd').next().next().show();
									error++;
								}else{
									jQuery('#form1').find('#vehiclereg_dd,#vehiclereg_mm,#vehiclereg_yy').removeClass('error');
									jQuery('#form1').find('#vehiclereg_dd').next().next().hide();
								}
							}
						}
					}
	...
	if(jQuery('#form1').find('#vehicle_insur').val() == ''){
		...
		if(jQuery('#form1').find('#vehicleins_dd').length > 0){
			//alert();
			if(jQuery('#form1').find('#vehicleins_dd').val() == ''){
				jQuery('#form1').find('#vehicleins_dd').addClass('error');
				jQuery('#form1').find('#vehicleins_mm').addClass('error');
				jQuery('#form1').find('#vehicleins_yy').addClass('error');
				jQuery('#form1').find('#vehicleins_dd').next().show();
				error++;
			}else{
				jQuery('#form1').find('#vehicleins_dd').removeClass('error');
				jQuery('#form1').find('#vehicleins_dd').next().hide();
				if(jQuery('#form1').find('#vehicleins_mm').length > 0){
					//alert();
					if(jQuery('#form1').find('#vehicleins_mm').val() == ''){
						jQuery('#form1').find('#vehicleins_mm').addClass('error');
						jQuery('#form1').find('#vehicleins_mm').next().show();
						error++;
					}else{
						jQuery('#form1').find('#vehicleins_mm').removeClass('error');
						jQuery('#form1').find('#vehicleins_mm').next().hide();
						if(jQuery('#form1').find('#vehicleins_yy').length > 0){
							//alert();
							if(jQuery('#form1').find('#vehicleins_yy').val() == ''){
								jQuery('#form1').find('#vehicleins_yy').addClass('error');
								jQuery('#form1').find('#vehicleins_yy').next().show();
								error++;
							}else{
								jQuery('#form1').find('#vehicleins_yy').removeClass('error');
								jQuery('#form1').find('#vehicleins_yy').next().hide();
								var driverlicdate = jQuery('#form1').find('#vehicleins_dd').val()+'/'+jQuery('#form1').find('#vehicleins_mm').val()+'/'+jQuery('#form1').find('#vehicleins_yy').val();
								if(checkdateno(driverlicdate) == 1 && driverlicdate!=''){
									jQuery('#form1').find('#vehicleins_dd,#vehicleins_mm,#vehicleins_yy').addClass('error');
									jQuery('#form1').find('#vehicleins_dd').next().next().show();
									error++;
								}else{
									jQuery('#form1').find('#vehicleins_dd,#vehicleins_mm,#vehicleins_yy').removeClass('error');
									jQuery('#form1').find('#vehicleins_dd').next().next().hide();
								}
							}
						}
					}
	...
	if(jQuery('#form1').find('#tafweeth_img').val() != ''){
		if(jQuery('#form1').find('#tafw_dd').length > 0){
			//alert();
			if(jQuery('#form1').find('#tafw_dd').val() == ''){
				jQuery('#form1').find('#tafw_dd').addClass('error');
				jQuery('#form1').find('#tafw_mm').addClass('error');
				jQuery('#form1').find('#tafw_yy').addClass('error');
				jQuery('#form1').find('#tafw_dd').next().show();
				error++;
			}else{
				jQuery('#form1').find('#tafw_dd').removeClass('error');
				jQuery('#form1').find('#tafw_dd').next().hide();
				if(jQuery('#form1').find('#tafw_mm').length > 0){
					//alert();
					if(jQuery('#form1').find('#tafw_mm').val() == ''){
						jQuery('#form1').find('#tafw_mm').addClass('error');
						jQuery('#form1').find('#tafw_mm').next().show();
						error++;
					}else{
						jQuery('#form1').find('#tafw_mm').removeClass('error');
						jQuery('#form1').find('#tafw_mm').next().hide();
						if(jQuery('#form1').find('#tafw_yy').length > 0){
							//alert();
							if(jQuery('#form1').find('#tafw_yy').val() == ''){
								jQuery('#form1').find('#tafw_yy').addClass('error');
								jQuery('#form1').find('#tafw_yy').next().show();
								error++;
							}else{
								jQuery('#form1').find('#tafw_yy').removeClass('error');
								jQuery('#form1').find('#tafw_yy').next().hide();
								var driverlicdate = jQuery('#form1').find('#tafw_dd').val()+'/'+jQuery('#form1').find('#tafw_mm').val()+'/'+jQuery('#form1').find('#tafw_yy').val();
								if(checkdateno(driverlicdate) == 1 && driverlicdate!=''){
									jQuery('#form1').find('#tafw_dd,#tafw_mm,#tafw_yy').addClass('error');
									jQuery('#form1').find('#tafw_dd').next().next().show();
									error++;
								}else{
									jQuery('#form1').find('#tafw_dd,#tafw_mm,#tafw_yy').removeClass('error');
									jQuery('#form1').find('#tafw_dd').next().next().hide();
								}
							}
						}
					}
	...
	if(jQuery('#form1').find('#iban').val() == ''){
		...
		//var regex = /^([a-zA-Z\u0600-\u06FF]{2}[0-9\u0660-\u0669]{22})+$/;
...
function getYears(from) {
	...
	if(datearr[1] > 9){
		if(datearr[0] > 9){
			...
			from = datearr[2]+'-'+datearr[1]+'-0'+datearr[0];
		}
	}else{
		if(datearr[0] > 9){
			from = datearr[2]+'-0'+datearr[1]+'-'+datearr[0];
		}else{
			from = datearr[2]+'-0'+datearr[1]+'-0'+datearr[0];
		}
		
	}
	console.log(from);
	var createDate = from+' 09:09:38Z';
	createDate = createDate.replace(" ", "T");
	createDate = new Date(createDate);
    //var d1 = new Date(from +' 12:00:00'),
    var d1 = createDate;
        d2 = new Date(),
        yr = [],
		tyr = 0;
...
jQuery('#user_regs').click(function(e){
	...
	if(!regex.test(jQuery('#user_mname').val()) && jQuery('#user_mname').val()!=''){
		jQuery('#user_mname').addClass('error');
		jQuery('#user_mname').next().show();
		error++;
	}else{
		jQuery('#user_mname').removeClass('error');
		jQuery('#user_mname').next().hide();
	}
...
var myval = '';
			//if(!jQuery('#user_mobile').val().match('[0-9\u0660-\u0669]{9}')){
            $.ajax({
           url: 'https://najez-online.com/api/rest_api/check_mobile_no/',
           type: 'GET',
		   async:false,
           ContentType: 'application/json',
           data: {'code': jQuery('#user_mobile').val()},
           success: function (response) {
           var datas = JSON.parse(response);
			myval=datas.data;
	...
	*/
	...
	setTimeout(function() {
		jQuery('.msgpop,.msgpop1').hide();
	}, 10000);
});

//$(document).ready(function(){
        var itaImgLink = "http://najez-online.com/assets/home/img/saudi-arabia.jpg";
		var deuImgLink = "http://najez-online.com/assets/home/img/urdu.jpg";
    	var engImgLink = "http://najez-online.com/assets/home/img/Grossbritanien.jpg";
